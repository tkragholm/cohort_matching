...
fn group_anchors_by_strata<R: MatchingRecord>(
    ...
    // Groups come back in FIRST-APPEARANCE order, which is the anchors' own order.
    //
    // This used to be `groups.into_values().collect()` over a `std::HashMap`, whose
    // `RandomState` hasher is seeded per PROCESS -- so the group order, and with it
    // the order of the pairs collected from them, differed on every run.
    //
    // That is not cosmetic for a caller who resamples. Measured on a downstream
    // epidemiological pipeline: two identical runs produced a matched cohort of the
    // same SIZE with the same MEMBERSHIP in a different row order, and because its
    // clustered bootstrap draws in the order rows arrive, 38 of 40 analysis slices
    // then reported a different point estimate. A published figure could not be
    // regenerated by re-running the command that produced it.
    //
    // First appearance rather than sorting the key: it needs no ordering semantics
    // for `Vec<Option<String>>`, it costs one extra map lookup, and it makes the
    // output order a function of the INPUT order -- which the caller controls and can
    // therefore make canonical.
    let mut index_of: HashMap<Vec<Option<String>>, usize> = HashMap::new();
    ...
    for (anchor_idx, case) in anchors.iter().enumerate() {
        ...
        match index_of.get(&key) {
            Some(&position) => groups[position].push(anchor_idx),
            None => {
                index_of.insert(key, groups.len());
                groups.push(vec![anchor_idx]);
            }
        }
    }
    groups
...
#[cfg(test)]
mod strata_group_order_tests {
    use super::{TrimmedIndexConfig, group_anchors_by_strata};
    use crate::types::BaseRecord;
    use chrono::NaiveDate;

    fn anchor(id: &str, site: &str) -> BaseRecord {
        let mut record =
            BaseRecord::new(id, NaiveDate::from_ymd_opt(2005, 1, 1).expect("valid date"));
        record.strata.insert("site".to_string(), site.to_string());
        record
    }

    /// Strata groups come back in first-appearance order, in every process.
    ///
    /// This was `HashMap::into_values()`, and `std`'s `RandomState` is seeded per
    /// PROCESS -- so the group order, and with it the order of the pairs collected
    /// from those groups, differed on every run. Downstream that moved the row
    /// order of the matched cohort, and the clustered bootstrap draws in the order
    /// rows arrive, so two identical pipeline runs reported different intervals.
    ///
    /// A single process cannot observe the old bug: its hasher seed is fixed for
    /// its lifetime, so a test that merely ran this twice would pass either way.
    /// So this asserts the POSITIVE property instead -- the order is the anchors'
    /// own -- which one process can check and which a random hasher will not
    /// satisfy by luck across three interleaved strata.
    #[test]
    fn groups_follow_first_appearance_of_their_strata() {
        // Interleaved on purpose: first appearances are b, a, c.
        let anchors: Vec<BaseRecord> = ["b", "a", "c", "a", "b", "c", "a"]
            .iter()
            .enumerate()
            .map(|(i, site)| anchor(&format!("p{i}"), site))
            .collect();
        let required = vec!["site".to_string()];
        let trimmed = TrimmedIndexConfig {
            anchors: None,
            candidates: None,
        };

        let groups = group_anchors_by_strata(&anchors, &required, &trimmed);

        assert_eq!(
            groups,
            vec![vec![0, 4], vec![1, 3, 6], vec![2, 5]],
            "groups must follow first appearance (b, a, c), each keeping anchor order"
        );
    }
}
