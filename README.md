...
- **Named Numeric Calipers**: `caliper_on_field("mother_birth_year", 1.0)` builds a constraint from a `(field, window)` pair. A record missing the field refuses the pair.
- **Run-Time Constraint Lists**: `.with_constraints(vec)` appends however many constraints the caller has, so matching rules can come from configuration rather than from code.
